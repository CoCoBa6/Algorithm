...
class Solution72411 {
    fun solution(orders: Array<String>, course: IntArray): Array<String> {
        ...
        orders.forEach { order ->
            order.forEach { menu -> menuSet.add(menu.toString()) }
        }

        val test = mutableListOf<List<String>>()

        course.forEach { targetNum ->
            var maxNum = 0
            combination(
                answer = test,
                elements = menuSet,
                visited = BooleanArray(menuSet.size) { false },
                start = 0,
                target = targetNum
            )

            test.forEach { combination ->
                var num = 0
                orders.forEach { order ->
                    var i = 0
                    run combination@ {
                ...
                if(num > 1 && num > maxNum){
                    answerList.clear()
                    answerList.add(combination.joinToString(""))
                    maxNum = num
                }
    ...
    private fun combination(answer: MutableList<List<String>>, elements: HashSet<String>, visited: BooleanArray, start: Int, target: Int) {
        if(target == 0) {
