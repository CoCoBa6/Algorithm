package kakao.level_4

import java.util.ArrayList

// [PGS] 매출 하락 최소화 / level_4 / 180분
...
https://grepp-programmers.s3.ap-northeast-2.amazonaws.com/files/production/a55bfc1d-951e-478f-bbe8-f5eb021e5a0d/2021_kakao_sales_01.png
...
CEO를 포함하여 모든 직원은 팀장 또는 팀원이라는 직위를 가지고 있으며 그림에서는 팀장과 팀원의 관계를 화살표로 표시하고 있습니다.
...
3-3. 한 직원은 최대 2개의 팀에 소속될 수 있습니다.
     만약 어떤 직원이 두 개의 팀에 소속되어 있다면, 반드시 하나의 팀에서는 팀장, 나머지 팀에서는 팀원이어야 합니다.
...
해당 직원들의 하루평균 매출액의 합은 44(14+13+17)원 입니다.
10번 직원은 C팀과 D팀 모두에 속해 있으므로, 두 팀에서 모두 참석한 것으로 인정됩니다.
...
[입출력 예]
sales	                                    links	                                                        result
[14, 17, 15, 18, 19, 14, 13, 16, 28, 17]	[[10, 8], [1, 9], [9, 7], [5, 4],                               44
...
class Solution72416 {
    private lateinit var saleList: MutableList<Int>
    private lateinit var cost: MutableList<MutableList<Int>>
    private lateinit var adj: MutableList<ArrayList<Int>>

    fun solution(sales: IntArray, links: Array<IntArray>): Int {
        saleList = sales.toMutableList()
        cost = MutableList(sales.size) { MutableList(2) { 0 } }
        adj = MutableList(saleList.size) { ArrayList() }

        links.forEach { adj[it[0] - 1].add(it[1] - 1) }

        dfs(0)

        return cost[0][0].coerceAtMost(cost[0][1])
    }

    private fun dfs(index: Int) {
        if(this::saleList.isInitialized && this::cost.isInitialized && this::adj.isInitialized){
            // 참석 X index = 0, 참석 O index = 1
            cost[index][0] = 0
            cost[index][1] = saleList[index]

            if (adj[index].isEmpty())
                return  // 자식이 없으므로 return
            ...
            adj[index].forEach {
                ...
                // child가 참여 안 하는게 이득인 경우
                if (cost[it][0] < cost[it][1]) {
                    cost[index][0] += cost[it][0]
                    cost[index][1] += cost[it][0]

                    // (팀장 팀원 모두 안 갈 경우의 수 제거) 최소 비용 추가
                    max = max.coerceAtMost(maximumValue = cost[it][1] - cost[it][0])
                } else {
                    // child가 참여 하는게 이득인 경우
                    cost[index][0] += cost[it][1]
                    cost[index][1] += cost[it][1]
                    max = 0
                }
            }
