...
// [PGS] 광고 삽입 / Level_3 / 80분

/**
[문제 설명]

카카오TV에서 유명한 크리에이터로 활동 중인 죠르디는
환경 단체로부터 자신의 가장 인기있는 동영상에 지구온난화의 심각성을 알리기 위한 공익광고를 넣어 달라는 요청을 받았습니다.
평소에 환경 문제에 관심을 가지고 있던 "죠르디"는 요청을 받아들였고 광고효과를 높이기 위해
...
class Solution72414 {
    fun solution(play_time: String, adv_time: String, logs: Array<String>): String {
        val playTime = convert(play_time)
        val advTime = convert(adv_time)
        val total = IntArray(playTime)

        logs.forEach { log ->
            val (startTime, endTime) = log.split("-").map(this::convert)
        ...
        for (i in advTime until playTime){
            currentTime = currentTime + total[i] - total[i - advTime]
            if (maxTime < currentTime){
                maxTime = currentTime
                startMaxTime = i - advTime + 1
            }
        }

        return String.format("%02d:%02d:%02d", startMaxTime/3600, startMaxTime/60 % 60, startMaxTime % 60)
    }

    private fun convert(time: String): Int {
        val (hh, mm, ss) = time.split(":").map { it.toInt() }
        return (hh * 3600) + (mm * 60) + ss
    }

    // 똑같은 코드인데 왜 시간초과가 되는거야
//    fun solution(play_time: String, adv_time: String, logs: Array<String>): String {
//        val playTime = convert(play_time)
//        val advTime = convert(adv_time)
//        val total = IntArray(playTime)
...
//            val (startTime, endTime) = log.split("-").map(this::convert)
//
//            (startTime until endTime).forEach {
//                total[it] += 1
//            }
//        }
//
//        var currentTime = 0L
//        (0 until advTime).forEach { currentTime += total[it] }
//        var startMaxTime = 0
//        var maxTime = currentTime
//        (advTime + 1 until playTime).forEach {
//            currentTime = currentTime + total[it] - total[it - advTime]
//            if(currentTime > maxTime){
//                maxTime = currentTime
//                startMaxTime = it - advTime + 1
//            }
//        }
//
//        return String.format("%02d:%02d:%02d", startMaxTime/3600, startMaxTime/60 % 60, startMaxTime % 60)
...
fun main() {
    val solution72414 = Solution72414()
    println(
        solution72414.solution(
            play_time = "02:03:55",
            adv_time = "00:14:15",
            logs = arrayOf(
                "01:20:15-01:45:14",
