...
인재영입팀에 근무하고 있는 니니즈는 코딩테스트 결과를 분석하여,
채용에 참여한 개발팀들에 제공하기 위해 지원자들의 지원 조건을 선택하면 해당 조건에 맞는 지원자가 몇 명인 지 쉽게 알 수 있는 도구를 만들고 있습니다.
...
senior 경력이면서, 소울푸드로 chicken을 선택한 사람 중 코딩테스트 점수를 100점 이상 받은 사람은 모두 몇 명인가?
...
class Solution72412 {
    fun solution(info: Array<String>, query: Array<String>): IntArray {
        val map = HashMap<Info, MutableList<Int>>()
        val answer = mutableListOf<Int>()
        info.forEach {
            val (lang, job, career, food, score) = it.split(" ")
            mutableListOf("-", lang).forEach { langList ->
                mutableListOf("-", job).forEach { jobList ->
                    mutableListOf("-", career).forEach { careerList ->
                        mutableListOf("-", food).forEach { foodList ->
                            val info1 = Info(langList, jobList, careerList, foodList)

                            if (map.containsKey(info1)){
                                val list = map[info1]
                                list!!.add(score.toInt())
                                map[info1] = list
                            } else {
                                map[info1] = mutableListOf(score.toInt())
                            }
                        }
                    }
                }
        ...
        map.keys.forEach { map[it]?.sort() }

        query.forEach {
            val (lang, job, career, food, score) = it.replace("and", "")
                .replace("[ ]*[ ]".toRegex(), " ")
                .split(" ")

            val list = map[Info(lang, job, career, food)]
            ...
            list?.let {
                ...
                while(start <= end) {
                    var mid = (start + end)/2
                    if(list[mid] < score.toInt()) start = mid + 1 else end = mid - 1
                }
                answer.add(list.size - start)
            }
            if (list == null) answer.add(0)
        }
